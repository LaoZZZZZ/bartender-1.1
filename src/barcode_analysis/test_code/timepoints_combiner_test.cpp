...
void drive(const vector<pair<string, string>>& cluster_files,
    ...
    std::shared_ptr<ClusterCenterLinkGenerator> link_generator(new ClusterCenterLinkGenerator(mixture_entropy_threshold,
                                                                                              ...
                                                                                              p_value,
                                                                                              0.3));
   std::shared_ptr<CenterRecalibrator> recal(new CenterRecalibrator(mixture_entropy_threshold, maximum_centers, p_value, 0.1));
    std::shared_ptr<MergeByCenters> center_merger(new MergeByCenters(recal));
    ...
    dumper.WriteToFileWithoutBarcodes(clusters, frequency_number_column);
