...
public:
	 ...
	 */
	OutFileBuf(const std::string& out, bool isInMem = false,bool binary = false) :
		name_(out.c_str()), cur_(0), closed_(false)
	{
	    if(!isInMem)
            out_ = fopen(out.c_str(), binary ? "wb" : "w");
        else{
            size_t size = 2*BUF_SZ;
            char *p = (char *)out.c_str();
            out_ = open_memstream(&p,&size);
        }
		if(out_ == NULL) {
    ...
    OutFileBuf(bool isInMem){
        if(isInMem){
            size_t size = 2*BUF_SZ;
            out_ = open_memstream(&dest_,&size);
            if(out_ == NULL) {
                std::cerr << "Error: Could not open alignment output file " << std::endl;
                throw 1;
            }
            closed_ = false;
            cur_ = 0;
	 ...
	 * Write a c++ string to the write buffer and, if necessary, flush.
	 */
	virtual void writeChars(const char * s, size_t len) {
		assert(!closed_);
		if(cur_ + len > BUF_SZ) {
			if(cur_ > 0) flush();
			if(len >= BUF_SZ) {
	 ...
	 */
	virtual void writeChars(const char * s) {
...
protected:
	...
	const char *name_;
	//char *name_;  //changed by luzhao
	FILE       *out_;
	size_t      cur_;
	char        buf_[BUF_SZ]; // (large) input buffer
	bool        closed_;
	char*       dest_;  //add by luzhao
};

class OutMemBuf : public OutFileBuf{
    public:
    OutMemBuf(char* dest,size_t size = 160*1024){
      this->name_ = NULL;

      this->closed_ = false;
      this->out_ = NULL;
      out_ = open_memstream(&dest,&size);

      this->fileBuf_ = new OutFileBuf();
      this->fileBuf_->setFile(out_);
    }

    //char* getResult() {return this->dest_;}
    ~OutMemBuf(){
	 ...
	 */
	virtual void writeChars(const char * s, size_t len) {
		this->fileBuf_->writeChars(s,len);
	}
