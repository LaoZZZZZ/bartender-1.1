 ...
 *
 * S2bDnaString:
 *   Like SString, but stores a list UINT32 words where each word is divided
...
public:
	...
	void set(int c, size_t idx) {
        ...
        cs_[word] = cs_[word] & ~(UINT32)(3 << bpoff);
        cs_[word] = cs_[word] |  (UINT32)(c << bpoff);
	}
	 ...
	 */
    void setWord(UINT32 w, size_t i) {
		assert_lt(i, nwords());
		cs_[i] = w;
	}
	 ...
	 */
    void install(const UINT32* b, size_t sz) {
		if(sz == 0) return;
		resize(sz);
        memcpy(cs_, b, sizeof(UINT32)*nwords());
	}
	...
	void reverse() {
		...
		while(wordf > wordi || (wordf == wordi && bpf > bpi)) {
            ...
            cs_[wordf] &= ~(UINT32)(3 << (bpf << 1));
            cs_[wordi] &= ~(UINT32)(3 << (bpi << 1));
            cs_[wordf] |=  (UINT32)(i << (bpf << 1));
            cs_[wordi] |=  (UINT32)(f << (bpi << 1));
			if(bpf == 0) {
				bpf = 15;
				wordf--;
			} else bpf--;
	...
	void fill(size_t len, char el) {
		...
		size_t bp = 0;
		for(size_t i = 0; i < len; i++) {
            cs_[word] &= ~(UINT32)(3  << (bp << 1));
            cs_[word] |=  (UINT32)(el << (bp << 1));
			if(bp == 15) {
				word++;
				bp = 0;
			} else bp++;
	 ...
	 * Return a const version of the raw buffer.
	 */
    const UINT32* buf() const { return cs_; }

	/**
	 * Return a writeable version of the raw buffer.
	 */
    UINT32* wbuf() { return cs_; }
...
extern UINT8 asc2dna[];
extern UINT8 asc2col[];
...
public:
		...
		bool crick = true)  // true -> possibly extract from Crick strand
	{
		...
		assert_gt(poss, 0);
        UINT32 rndoff = (UINT32)(rnd.nextU32() % poss);
		bool fw;
...
typedef SStringExpandable<char, 20, 2> BTString;
//typedef SDnaStringExpandable<1024, 2>    BTDnaString;
typedef SDnaStringExpandable<50, 2>    BTDnaString;
