...
int main() {
    ...
    shared_ptr<CenterRecalibrator> center_detector(new CenterRecalibrator(0.47, // entropy threshold
				       4, // maximum center 
				       0.05, // p-value
				       0.1)); // sequence error
    ////////////////////////////////////////////////////////
    ...
    kmer k1;
    freq f1 = 8;
    kmersBitwiseTransform::getInstance()->seq_2_bitwise(seq1, k1, 5);    
    const std::shared_ptr<DictatorCenter> center_1(new DictatorCenter(k1,5));
    std::shared_ptr<cluster> second_cluster(new dictatorCluster(center_1,{f1}));

    first_cluster->merge(*second_cluster);
    shared_ptr<IDGenerator> id_pool(new IDGenerator({first_cluster->ClusterID(), second_cluster->ClusterID()}));
    
    unique_ptr<ClusterSplitter> splitter(new ClusterSplitter(center_detector,id_pool)); 
    vector<shared_ptr<cluster>> splited_clusters = splitter->split(first_cluster);
    for (const auto& c : splited_clusters) {
	if (c.get()){
    		printFrequency(c->bpFrequency());
	}
    }
    assert(splited_clusters.size() == 2);
    ...
    kmer k2;
    freq f2 = 8;
    kmersBitwiseTransform::getInstance()->seq_2_bitwise(seq2, k2, 5);    
    const std::shared_ptr<DictatorCenter> center_2(new DictatorCenter(k2,5));
    std::shared_ptr<cluster> third_cluster(new dictatorCluster(center_2,{f2}));
    first_cluster->merge(*third_cluster);

    id_pool.reset(new IDGenerator({first_cluster->ClusterID(), second_cluster->ClusterID(),
						     third_cluster->ClusterID()}));
    
    splitter.reset(new ClusterSplitter(center_detector,id_pool)); 
    splited_clusters = splitter->split(first_cluster);
    for (const auto& c : splited_clusters) {
	if (c.get()){
    		printFrequency(c->bpFrequency());
	}
    }
    assert(splited_clusters.size() == 3);
    ...
    kmer k3;
    freq f3 = 8;
    kmersBitwiseTransform::getInstance()->seq_2_bitwise(seq3, k3, 5);    
    const std::shared_ptr<DictatorCenter> center_3(new DictatorCenter(k3,5));
    std::shared_ptr<cluster> four_cluster(new dictatorCluster(center_3,{f3}));
    third_cluster->merge(*four_cluster);

    id_pool.reset(new IDGenerator({four_cluster->ClusterID(),
				   third_cluster->ClusterID()}));
    
    splitter.reset(new ClusterSplitter(center_detector,id_pool)); 
    splited_clusters = splitter->split(third_cluster);
    for (const auto& c : splited_clusters) {
	if (c.get()){
    		printFrequency(c->bpFrequency());
	}
    }
    assert(splited_clusters.size() == 4);
...
{0,6,0,0}
    ...
    for (const auto& c : centers) {	
	string seq;
	kmersBitwiseTransform::getInstance()->bitwise_2_seq(c, seq, 20);
	cout << seq << endl;	
    }*/	
    return 0;
}
