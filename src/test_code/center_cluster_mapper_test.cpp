  ...
  //  printCenter(centers);
   ...
   // printCenter(centers);
    for (const auto& p : all_centers) {
		...
		assert(linker.getClusterByCenter(c)->ClusterID() ==  p.first);
	}
    } 
    linker.removeCluster(4);
    assert(linker.hasCluster(4) == false); 
    for (const auto& c : all_centers[4]) {
	assert(linker.hasCenter(c) == false);
    }
    assert(linker.getClusterByCenter(all_centers[3].front())->ClusterID() == 3);
    list<kmer> my_centers = linker.myCenter(3);
    centers.assign(my_centers.begin(), my_centers.end());
    assert(centers == all_centers[3]);
    printCenter(centers); 
    return 0;
}
