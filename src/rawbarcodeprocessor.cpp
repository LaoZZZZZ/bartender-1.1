    ...
    void RawBarcodeProcessor::parseFile() {
        ...
        while (_csv_reader->nextRow(&row)) {
            ...
            while (_barcode_tables.size() < blen + 1) {
                _barcode_tables.push_back(barcodeFreqTable());
                _barcode_2_line.push_back(barcodeToPrimers());
            }
        ...
        for (auto& table : _barcode_2_line) {
            for (auto& k_primer : table) {
                sort(k_primer.second.begin(), k_primer.second.end());
            }
        }
    }
