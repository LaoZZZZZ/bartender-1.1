...
namespace barcodeSpace {
    ...
    double MeanEstimator::mean(std::list<std::shared_ptr<BarcodeCluster> > &clusters) {
        
        clusters.sort([](const std::shared_ptr<BarcodeCluster>& c1,
                         const std::shared_ptr<BarcodeCluster>& c2) {return c1->size() < c2->size();});
